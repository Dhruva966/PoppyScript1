...
function Alertism(config) {
  ...
  if (config.backdrop !== undefined) {
    AlertismBackdrop.style.background = config.backdrop;
  } else {
    AlertismBackdrop.style.background = 'rgb(0 0 0 / 50%)';
  }
  
  if (config.position !== undefined) {
    AlertismBackdrop.classList.add('alert-' + config.position);
  } else {
    AlertismBackdrop.className = 'alert-backdrop';
  }
  
  AlertismAlertContainer.style.background = config.background || '#fff';
  
  if (config.enableCloseBtn !== undefined) {
    if (config.enableCloseBtn === true) {
  ...
  if (config.icon !== undefined) {
    if (config.icon.type !== undefined) {
      AlertismIcon.innerHTML = checkIcon[config.icon.type + 'Icon'];
    }
    if (config.icon.color !== undefined) {
      document.querySelectorAll('.alert-backdrop .alert-container svg path').forEach((path) => {
        path.style.fill = config.icon.color;
      });
      if (config.icon.type === 'warning') {
        document.querySelector('.alert-backdrop .alert-container svg circle').style.fill = config.icon.color;
      }
    }
  ...
  if (config.alertHeading !== undefined) {
    AlertismHeading.innerText = config.alertHeading;
  } else {
    AlertismHeading.innerText = 'Alert';
  }
  
  if (config.alertText !== undefined) {
    AlertismContent.innerText = config.alertText;
  } else {
    if (config.alertHTML !== undefined) {
      AlertismContent.innerHTML = config.alertHTML;
    } else {
  ...
  if (config.enableInput !== undefined) {
    if (config.enableInput === true) {
  ...
  if (config.input !== undefined) {
    if (config.input.type !== undefined) {
      AlertismInput.type = config.input.type;
      if (config.input.type === 'textarea') {
        AlertismInput.parentNode.replaceChild(AlertismTextarea, AlertismInput);
      }
    }
    if (config.input.value !== undefined) {
      AlertismInput.value = config.input.value;
      AlertismTextarea.value = config.input.value;
    }
    if (config.input.placeholder !== undefined) {
      AlertismInput.placeholder = config.input.placeholder;
      AlertismTextarea.placeholder = config.input.placeholder;
    }
  } else {
    AlertismInput.type = 'text';
    AlertismInput.value = '';
  ...
  if (config.btnText !== undefined) {
    AlertismOkBtn.innerText = config.btnText;
  } else {
    AlertismOkBtn.innerText = 'Ok';
  }
  
  if (config.closeBtnText !== undefined) {
    AlertismCloseBtn.classList.remove('alert-hide');
    AlertismCloseBtn.innerText = config.closeBtnText;
  } else {
    AlertismCloseBtn.classList.add('alert-hide');
    AlertismCloseBtn.innerText = '';
  }
  
  if (config.alertFooter !== undefined) {
    AlertismFooter.classList.remove('alert-hide');
    AlertismFooter.innerHTML = config.alertFooter;
  } else {
    AlertismFooter.classList.add('alert-hide');
    AlertismFooter.innerHTML = '';
  }
  
  if (config.animation !== undefined) {
    AlertismAlertContainer.style.animation = config.animation + ' 0.5s linear 1';
  } else {
    if (config.animationIn !== undefined) {
      AlertismAlertContainer.style.animation = config.animationIn + ' 0.5s linear 1';
    } else {
  ...
  AlertismOkBtn.onclick = () => {
    closeAlertism(config.animation, config.animationOut, config.onConfirmed);
  };
  
  AlertismCloseBtn.onclick = () => {
    closeAlertism(config.animation, config.animationOut, config.onDenied);
  };
  
  AlertismCrossBtn.onclick = () => {
    closeAlertism(config.animation, config.animationOut, config.onCanceled);
  };
  ...
  if (config.theme !== undefined) {
    ...
    AlertismAlertContainer.classList.add('alert-' + config.theme);
  } else {
    AlertismAlertContainer.className = 'alert-container';
  }
  ...
  if (config.timer !== undefined) {
    ...
    AlertismTimerCountDown.style.animation = 'closeTimeLine ' + (Number(config.timer) / 1000) + 's linear';
    ...
    setTimeout(() => {
      closeAlertism(config.animation, config.animationOut, config.onAutoClose);
    }, Number(config.timer));
  } else {
    AlertismTimerCountDown.classList.add('alert-hide');
...
function closeAlertism(animation, animationOut, callback) {
  if (animation) {
    AlertismAlertContainer.style.animation = animation + '-out 0.5s linear 1';
  } else {
    if (animationOut) {
      AlertismAlertContainer.style.animation = animationOut + ' 0.5s linear 1';
    } else {
      AlertismAlertContainer.style.animation = '';
    }
  }
